...
namespace ServerlessImageToThumbnail.Adapter
{
    public class UploadFileMPUHighLevelAPITest
    {
        ...
        //{
        //    s3Client = new AmazonS3Client(AccesskeyID, SecretKey, bucketRegion);
        //    UploadFileAsync().Wait();
        //}

        public static async Task UploadFileAsync(Stream stream)
        {
            try
            {
                s3Client = new AmazonS3Client(AccesskeyID, SecretKey, bucketRegion);
                var keyName = Guid.NewGuid().ToString() + ".png";
